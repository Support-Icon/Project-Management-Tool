...
const systemPromptFor = (user) => {
  ...
  return [
    ...
    'When creating/assigning tasks or projects, ask ONE missing question at a time (project name, task title, assignee, start date, due date, optional depends-on task title). Do not dump a long form.',
    'A task can start after another task completes — use dependsOnTaskTitle when the user wants sequencing.',
    ...
    'For daily updates, only the assignee can submit, and only for In Progress tasks (not To Do).',
